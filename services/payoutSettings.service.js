...
const { PoolPayoutSettings, Pools } = require('../models/index.js');
...
async function validatePayoutAmount(poolId, amount) {
    try {
        ...
        const Pools = await Pools.findByPk(poolId);
        ...
        if (amount > Pools.total_contributed) {
...
async function checkDailyPayoutLimit(poolId) {
    try {
        ...
        const todayPayoutsCount = await sequelize.models.Payout.count({
            where: {
                poolID: poolId,
                created_at: {
